...
const createToken = (user) => {
    ...
    const accessToken = jwt.sign(payload, process.env.JWT_SECRET_KEY, { expiresIn: process.env.ACCESS_TOKEN_EXPIRY });
    const refreshToken = jwt.sign(payload, process.env.JWT_SECRET_KEY, { expiresIn: process.env.REFRESH_TOKEN_EXPIRY });
...
export const signUpUser = async (req, res) => {
    ...
    if (!name || !email || !password || !role) {
        return res.status(400).send({ success: false, message: "Enter all the fields" });
    }
    try {
...
export const login = async (req, res) => {
    ...
    if (!email || !password) {
        return res.status(401).send({ success: false, message: "Missing credentials" });
    }
    try {
